...
async def create_schema(
    ...
    """
    Create a schema definition from a document that can be used to initialize the database.
...
@app.get("/download-schemas")
async def download_schemas() -> JSONResponse:
    """
    Download all existing schemas as JSON format compatible with schema initializer.
    Returns an array of schema definitions that can be saved as individual JSON files.
    """
    try:
        async with db.async_session_factory() as session:
            stmt = select(DocumentSchema)
            ...
            return JSONResponse(
                ...
                content={
                    "status": "success",
                    "message": f"Retrieved {len(schema_definitions)} schema definitions",
                    "schema_definitions": schema_definitions,
                    "usage_instructions": {
                        "description": "Save each schema definition as a separate JSON file in the schemas/ directory",
                        "filename_format": "{document_type}.json",
                        "example_save": "Save each object in the schema_definitions array as schemas/{document_type}.json",
                        "reload_instructions": "Restart the application to load schemas from the updated files"
                    }
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to download schemas: {e}")


@app.get("/download-schema/{schema_id}")
async def download_single_schema(schema_id: str) -> JSONResponse:
    """
    Download a single schema by ID as JSON format compatible with schema initializer.
    """
    try:
        async with db.async_session_factory() as session:
            stmt = select(DocumentSchema).where(DocumentSchema.id == uuid.UUID(schema_id))
            ...
            return JSONResponse(
                ...
                content={
                    "status": "success",
                    "message": f"Schema definition for {schema.document_type} ({schema.country})",
                    "schema_definition": schema_definition,
                    "usage_instructions": {
                        "description": "Save this schema_definition object as a JSON file in the schemas/ directory",
                        "filename_suggestion": f"{schema.document_type.lower()}.json",
                        "example_usage": f"Save as schemas/{schema.document_type.lower()}.json and restart the application to load it automatically"
                    }
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to download schema: {e}")


@app.get("/download-schemas/zip")
async def download_schemas_zip() -> FileResponse:
    """
    Download all schemas as a zip file containing individual JSON files.
    Each schema is saved as a separate JSON file in the zip archive.
    """
    try:
        async with db.async_session_factory() as session:
            stmt = select(DocumentSchema)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            zip_path = os.path.join(temp_dir, "schemas.zip")
            ...
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                for schema in schemas:
                    ...
                    # Create filename from document type and country
                    filename = f"{schema.document_type.lower().replace(' ', '_')}_{schema.country.lower().replace(' ', '_')}.json"
                    
                    # Add schema to zip
                    zipf.writestr(filename, json.dumps(schema_definition, indent=2))
            
            # Create a permanent copy for FileResponse
            final_zip_path = tempfile.mktemp(suffix=".zip")
            shutil.copy2(zip_path, final_zip_path)
            
            # Return FileResponse without background task - file will be cleaned up by OS
            return FileResponse(
                path=final_zip_path,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create schemas zip: {e}")
