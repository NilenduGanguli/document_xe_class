...
async def get_field_list_from_documents_openai(
    document_paths: List[Path],
    document_types: List[str],
    document_type: str,
    country: str
    ...
    try:
        ...
        return ExtractedFields(**parsed_data).field_names
    except Exception as e:
        logger.error(f"Field extraction failed: {e}")
        return None


async def generate_schema_from_documents_openai(
    document_paths: List[Path],
    document_types: List[str],
    document_type: str,
    country: str
    ...
    try:
        field_extraction_task = get_field_list_from_documents_openai(
            document_paths, document_types, document_type, country
        )

        # Prepare user content for schema generation (same as field extraction)
        user_content = []
        for doc_path, content_type in zip(document_paths, document_types):
            if not doc_path.exists():
                continue
        ...
        field_names = await field_extraction_task

        if not field_names or not user_content:
            logger.error("Failed to get field names or document parts")
            return None
    ...
    for attempt in range(SCHEMA_GENERATION_RETRY_ATTEMPTS):
        try:
            logger.info(f"Sending schema generation request to OpenAI (Attempt {attempt + 1})")
            response = await client.chat.completions.create(
                model="gpt-4o",
                messages=[
                ...
                response_format={
                    "type": "json_schema",
                    "json_schema": {
                        "name": "schema_generation_response",
                        "schema": GeneratedSchema.model_json_schema(),
                        "strict": True
                    }
                },
                temperature=0.0
            ...
            import json
            parsed_data = json.loads(content)
            parsed_response = GeneratedSchema(**parsed_data)
