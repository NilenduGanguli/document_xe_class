...
async def extract_with_db_schema_openai(
    document_paths: List[Path],
    document_types: List[str],
    document_schema: DocumentSchema,
    attempt: int = 0
    ...
    try:
        user_content = []
        user_content.append({"type": "text", "text": "Please extract the information from the provided documents according to the schema."})
    ...
    for retry_attempt in range(EXTRACTION_RETRY_ATTEMPTS + 1):
        try:
            logger.info(f"Sending extraction request to OpenAI (Attempt {retry_attempt + 1})")
            ...
            validated_data = pydantic_model(**parsed_data)
